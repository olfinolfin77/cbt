...
if(isset($_POST['pilihan_jurusan'])){
    ...
    $result_daya_tampung = pg_query($query);
    $row = pg_fetch_array($result_daya_tampung);
...
string_agg(d5.nilai_per_kategori, ',') as nilai_kategori from (
SELECT d4.*,concat(nama_kategori::text,'='::text,nilai_kategori::text) as nilai_per_kategori from (
...
) as d1 group by no_peserta,nama,alamat,telepon,nilai,keterangan,fill_nilai,pilihan_jurusan1,pilihan_jurusan2 ORDER BY nama
...
) as d5 where pilihan_jurusan1='{$_POST['pilihan_jurusan']}' or pilihan_jurusan2='{$_POST['pilihan_jurusan']}' group by no_peserta,nama,alamat,telepon,nilai,keterangan,fill_nilai,pilihan_jurusan1,pilihan_jurusan2
